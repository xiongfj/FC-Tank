 ...
 *
 * Welcome to the irrKlang API documentation(文档). This page should give you a short overview(概述) 
 * over irrKlang, the high level audio library(库). 

 * In this documentation files you'll find any information you'll need to develop applications(应用) with
 * irrKlang using C++. If you are looking for a tutorial(个别辅导) on how to start, you'll
 ...
 *
 * The irrKlang library is intended(打算) to be an easy-to-use 3d and 2d sound engine, so
 * this documentation is an important part of it. If you have any questions or
 * suggestions, please take a look into the ambiera.com forum(论坛) or just send a mail.
 *
 * <br/>
 * <br/>
 *
 ...
 * cross platform(平台) sound engine and audio library.

 * It has a very simply object orientated(定向) interface(接口) and was designed to be used
 * in games, scientific simulations(模拟), architectural(结构的) visualizations(形象化的) and similar.
 * irrKlang plays several file formats(格式) such as
 * <ul>
 *               <li>RIFF WAVE (*.wav)</li>
 ...
 * </ul>
 * It is also able to run on different operating(操作) systems and use several output drivers:
 *<ul>
 ...
 *
 * To be more flexible(灵活的) playing back sounds, irrKlang uses the concept(感念) of sound sources. 
 ...
 * \endcode
 *
 * The advantage(好处) of using irrklang::ISoundSource is that it is possible to set 
...
namespace irrklang
{
	//! Creates an irrKlang device(装置). The irrKlang device is the root object for using the sound engine.

	// 确定声音驱动
	/** \param driver(驱动) The sound output driver to be used for sound output. Use irrklang::ESOD_AUTO_DETECT
	to let irrKlang decide(决定) which driver will be best.

	// 打开声音引擎的方式
	\param options A combination(组合) of irrklang::E_SOUND_ENGINE_OPTIONS literals(irrklang 命名空间里的一个枚举型常量名). Default value is 
	irrklang::ESEO_DEFAULT_OPTIONS.

	// 额外选项
	\param deviceID Some additional(额外的) optional(选项) deviceID(编号) for the audio driver(声音驱动). If not needed, simple
	set this to 0. 
	This can be used for example to set a specific(特定的) ALSA(驱动集) output pcm(调制) device(装备) for output
	("default" or "hw", for example). For most driver types, available(有效地) deviceIDs can be 
	...
	IRRKLANG_SDK_VERSION, which is done by default. This is needed for sdk version(版本) checks.
	...
	*/
	//创建一个 ISoundEngine* 的指针 p 即可用它来存储该函数的返回值了。p-> 就可以调用类 ISoundEngine 里面的播放声音函数播放声音了

	IRRKLANG_API ISoundEngine* IRRKLANGCALLCONV createIrrKlangDevice(
		E_SOUND_OUTPUT_DRIVER driver = ESOD_AUTO_DETECT,	// 在 ik_ESoundOutputDrivers.h 内的枚举型常量中定义。
		int options = ESEO_DEFAULT_OPTIONS,		// ik_ESoundEngineOptions.h 内的枚举型常量中有定义
		const char* deviceID = 0,
		const char* sdk_version_do_not_use = IRR_KLANG_VERSION);	// 该默认值的定义就在该头文件前面部分："1.4.0"
