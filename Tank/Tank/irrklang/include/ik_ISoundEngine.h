...
namespace irrklang
{
	...
	*/
	...
	public:
		...
		// 第一个参数 soundFileNam 是声音文件的名字，是用引号引起来的字符串，如果有路径，必须写上相对或绝对路径
		/** \param sourceFileName Filename of sound, like "sounds/test.wav" or "foobar.ogg".

		// 第二个参数 playLooped 是循环播放模式 (true 循环 / false 播放一次)。						//	播放完后停止并从内部播放列表删除它
		 ...
		 \param startPaused starts the sound paused. This implies(意味着) that track=true. Use this if you want to modify(修改) some of the playing
		 parameters(参数) before the sound actually(实际上) plays. Usually you would set this parameter to true, then use the ISound interface(接口) to
		 ...
		 \param streamMode Specifies if the file should be streamed(流动) or loaded completely into memory for playing.
		 ESM_AUTO_DETECT sets this to autodetection(自动检测). Note: if the sound has been loaded or played before into the
		 engine, this parameter has no effect(作用).

		 \param enableSoundEffects(音效) Makes it possible to use sound effects such as chorus(合唱), distorsions(扭曲), echo(回声), 
		 reverb(电子声) and similar(类似) for this sound. Sound effects can then be controlled via(通过) ISound::getSoundEffectControl().
		 Only enable if necessary. 

		// 3,4,6 参数如果为 true 该函数就会返回一个指向 ISound 的指针
		 \return Only returns a pointer to an ISound if the parameters 'track', 'startPaused' or 
		 'enableSoundEffects' have been	 set to true.
		 
		// 如果该函数返回一个 ISound，你就要调用函数 ISound 的成员函数 drop() 释放内存
		 Note: if this method returns an ISound as result, 
		 you HAVE to call ISound::drop() after you don't need the ISound interface anymore. Otherwise this 
		 will cause memory waste. 

		// 如果346参数为true，而声音未播放，就返回0
		 This method also may return 0 altough(即使) 'track', 'startPaused' or 
		 'enableSoundEffects' have been set to true, if the sound could not be played.*/

		// 建立一个 ISound* 的指针接受该函数的返回值，就可以用该指针调用 ISound 类里面的 set..之类的函数设置相应这首歌曲的参数了。
		// 只有对应的参数为 true 才能对播放进行一些操作。
		virtual ISound* play2D(const char* soundFileName, 
							   bool playLooped = false,		// 循环播放与否
							   bool startPaused = false,	// 如果 = 1，则以暂停方式打开，就是说你还要调用 ISound 类里面的 setIsPaused(false) 函数开始播放。
							   bool track = false,			// 允许设置音量之类的与否
							   E_STREAM_MODE streamMode = ESM_AUTO_DETECT,
							   bool enableSoundEffects = false) = 0;	// 允许设置音效与否

		// 该函数是从资源文件里播放声音
		//! Plays a sound source as(模仿) 2D sound with its default(默认) settings stored in ISoundSource.
		/** An ISoundSource object will be created internally(内部地) when playing a sound the first time,
		or can be added with getSoundSource().

		\param source The sound source, specifiying(定义) sound file source and default settings for this file.
		...
		\param sourceFileName Filename of sound, like "sounds/test.wav" or "foobar.ogg".
		 ...
		 will cause memory waste. This method also may return 0 altough 'track', 'startPaused' or 
		 'enableSoundEffects' have been set to true, if the sound could not be played.*/
		// 必须4、5、7 至少有一个参数为 true 才能利用返回值设置这三个参数。
